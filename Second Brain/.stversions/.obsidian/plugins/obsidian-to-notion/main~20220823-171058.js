  ...
  upload() {
    return __async(this, null, function* () {
      ...
      if (markDownData) {
        ...
        if (res.status === 200) {
          new import_obsidian3.Notice(`${this.settings.langConfig["sync-success"]}${basename}`);
        } else {
          new import_obsidian3.Notice(`${this.settings.langConfig["sync-fail"]}${basename}`);
        }
      }
    });
  }
  getNowFileMarkdownContent(app) {
    return __async(this, null, function* () {
      ...
      if (nowFile) {
        ...
        new import_obsidian3.Notice(this.settings.langConfig["open-file"]);
        return;
      }
    });
  }
...
var SampleSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    const notionApiKye = new import_obsidian3.Setting(containerEl).setName("Notion API Token").setDesc("It's a secret").addText((text) => text.setPlaceholder("Enter your Notion API Token").setValue(this.plugin.settings.notionAPI).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.notionAPI = value;
      yield this.plugin.saveSettings();
    })));
    notionApiKye.controlEl.querySelector("input").type = "password";
    const notionDatabaseID = new import_obsidian3.Setting(containerEl).setName("Database ID").setDesc("It's a secret").addText((text) => text.setPlaceholder("Enter your Database ID").setValue(this.plugin.settings.databaseID).onChange((value) => __async(this, null, function* () {
